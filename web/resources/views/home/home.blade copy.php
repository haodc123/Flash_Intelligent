					...
					<div class="inner">
						...
						@for ($i = 0; $i < $g_special_num; $i++)
							...
							<div class="mySlides fade">
								<div class="video col">
									<div class="image fit">
										<img src="../images/thumb/{{ getPathThumb($g_hot[$i]->g_site, $g_hot[$i]->g_thumb) }}" onerror="this.onerror=null;this.src='../images/thumb/thumb_def.png';" alt="Game online {{ $gt_by_id[$g_hot[$i]->g_cat_1][0] }}" />
									</div>
									<p class="caption">
										<span class="title">{{ $g_hot[$i]->g_title }} </span>
										<span class="small"><br />By: {{ $g_hot[$i]->g_author }}</span>
										<span class="small"><br />⯈ <a href="cat/{{ $gt_by_id[$g_hot[$i]->g_cat_1][1] }}">{{ $gt_by_id[$g_hot[$i]->g_cat_1][0] }}</a></span>
									</p>
						...
						<!-- Tabbed Video Section -->
							<div class="flex flex-tabs">
								...
								@foreach ($gt_by_id as $key=>$gt_by_id_i)
									@if ($gt_by_id_i[2] == 11)
										<li class="home"><a href="#" data-id="{{ $key }}" data-tab="tab-{{ $key }}">{{ $gt_by_id_i[0] }}</a></li>
									@endif
								@endforeach
								...
								<div class="tabs">
									...
									@if (sizeof($gbc_1) > 0)
										@for ($i = 0; $i < sizeof($gbc_1); $i++)
											<div class="video col">
												<div class="image fit">
													<img src="../images/thumb/{{ getPathThumb($gbc_1[$i]->g_site, $gbc_1[$i]->g_thumb) }}" onerror="this.onerror=null;this.src='../images/thumb/thumb_def.png';" alt="Game online {{ $gt_by_id[$gbc_1[$i]->g_cat_1][0] }}" />
												</div>
												<p class="caption">
										...
										@endfor
										<a href="cat/{{ $gt_by_id[$gbc_1[0]->g_cat_1][1] }}" class="link-more">See more</a>
									@endif
										</div>
								...
								<div class="tabs">
									...
									@if (sizeof($gbc_2) > 0)
										@for ($i = 0; $i < sizeof($gbc_2); $i++)
											<div class="video col">
												<div class="image fit">
													<img src="../images/thumb/{{ getPathThumb($gbc_2[$i]->g_site, $gbc_2[$i]->g_thumb) }}" onerror="this.onerror=null;this.src='../images/thumb/thumb_def.png';" alt="Game online {{ $gt_by_id[$gbc_2[$i]->g_cat_1][0] }}" />
												</div>
												<p class="caption">
										...
										@endfor
										<a href="cat/{{ $gt_by_id[$gbc_2[0]->g_cat_1][1] }}" class="link-more">See more</a>
									@endif
										</div>
